...
load(load_filename);
[nT, nRx, nTx] = size(full_synth_data); 
tx_elmts = 1:1:nTx;
rxdata_h = full_synth_data(:,:,tx_elmts); 
clearvars full_synth_data;
...
reg = 1e-2; 1e-3; % Regularization Corresponding to Relative Time of Flight Error
...
roughening = (I+reg_smoothing*F'*F);
...
for sos_iter = 1:num_sos_iterations
    ...
    for z_idx = 1:numel(z)-1
        ...
        img_full_pre = reshape(reshape(permute(txdata_f .* conj(rxdata_f), [1,3,2]), ...
            [numel(x)*no_elements, numel(f)])*FT, [numel(x), no_elements, numel(t_delays)]);  
        img_full = permute(reshape(roughening\reshape(permute(img_full_pre, [2,1,3]), ...
            [no_elements, numel(x)*numel(t_delays)]), [no_elements, numel(x), numel(t_delays)]), [2,1,3]);

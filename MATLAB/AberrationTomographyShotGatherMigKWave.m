...
load_filename = '../Datasets/AbdominalMap3.mat';
load(load_filename);
[X, Z] = meshgrid(x, z);
...
dBrange = [-40, 0]; % Dynamic Range [decibels]
...
for sos_iter = 1:num_sos_iterations
    ...
    if sos_iter == 1 % Show the Following Images Only on the First Iteration
        ...
        Cexact = interp2(X, Z, C, Xrecon, Zrecon, 'spline');
        dslowness = 1./Cexact - 1/cbfm;
        dtimes = reshape(diff_times(path_integrate(dslowness(:))), size(aberration_times));
        xpos_idx = 80;
        % Visualize Sound Speed Map and Transducer Grid
        figure; imagesc(1000*X(1,:), 1000*Z(:,1), C); 
        xlabel('Lateral [mm]'); ylabel('Axial [mm]'); colorbar;
        hold on; plot(1000*xpos, zeros(size(xpos)), 'w.', 'Linewidth', 2); axis image;
        ...
        figure; subplot(1,2,1); imagesc(1000*x_msmts, 1000*z_msmts, dtimes(:,:,xpos_idx));
        xlabel('Lateral [mm]'); ylabel('Axial [mm]'); colorbar; axis image;
        title('Linearized Prediction');
        subplot(1,2,2); imagesc(1000*x_msmts, 1000*z_msmts, ...
            aberration_times(:,:,xpos_idx), ...
            [min(min(dtimes(:,:,xpos_idx))), max(max(dtimes(:,:,xpos_idx)))]);
        xlabel('Lateral [mm]'); ylabel('Axial [mm]'); 
        title('Actual Aberration Delay [s]'); colorbar; hold on;
    ...
    figure; subplot(1,2,1); imagesc(1000*X(1,:), 1000*Z(:,1), C); 
    colorbar; axis image; hold on;
    xlabel('Lateral [mm]'); ylabel('Axial [mm]'); 
    plot(1000*xpos, zeros(size(xpos)), 'w.', 'Linewidth', 2);
    plot(1000*X_MSMTS(:), 1000*Z_MSMTS(:), 'r.', 'Linewidth', 2);
    title('Ground-Truth Speed of Sound');
    xlim(1000*[min(x),max(x)]); ylim(1000*[min(z),max(z)]);
    subplot(1,2,2); imagesc(1000*xrecon, 1000*zrecon, Crecon, [min(C(:)),max(C(:))]); 
    colorbar; axis image; hold on;
    xlabel('Lateral [mm]'); ylabel('Axial [mm]'); 
    plot(1000*xpos, zeros(size(xpos)), 'w.', 'Linewidth', 2);
    plot(1000*X_MSMTS(:), 1000*Z_MSMTS(:), 'r.', 'Linewidth', 2);

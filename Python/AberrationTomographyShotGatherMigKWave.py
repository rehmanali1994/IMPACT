...
load_filename = '../Datasets/AbdominalMap4.mat';
...
fTx = 8e6; # frequency [Hz]
fBW = 12e6; # bandwidth at half maximum [Hz]
pulse_cutoff = 1e-3; # passband cutoff
ord = 50; # Strength of Anti-Aliasing Window 
# Parameters to Filter Time Delay Measurements
Fnum = 1.3; # f-number to limit acceptance angle
dmin = 0.002; # start depth [m]
dmax = 0.040; # end depth [m]
dwnsmp = 3; # downsample measurements for tomography
medfilt_kernel = np.array([9,9]); # Size of Median Filter
# B-Mode Image Parameters/Imaging Window
dBrange = np.array([-40, 0]); # Dynamic Range [decibels]
Nximg = 1001; Nzimg = 1001;
xlims = np.array([-10e-3, 10e-3]); 
zlims = np.array([16e-3, 38e-3]);
# Sound Speed Reconstruction Parameters
blur_z = gausswin(50, 4); # Z - Blurring Over Reconstruction Grid
blur_x = gausswin(50, 4); # X - Blurring Over Reconstruction Grid
...
reg = (1e-4)*(upsamp_x); # Regularization Corresponding to Relative Time of Flight Error
alpha = 0.25; # Relative Weighting of Noninformative Prior to Layered Prior
    # 1.0 for full noninformative prior; 0.0 for fully layered prior
num_sos_iterations = 5; # Number of Sound Speed Estimations/Iterations
...
for sos_iter in np.arange(num_sos_iterations):
    ...
    for z_idx in np.arange(z.size-1):
        ...
        img_full = np.reshape(np.dot(np.reshape(np.transpose(txdata_f * np.conj(rxdata_f), axes=(0,2,1)), 
    ...
    if sos_iter == 0: # Show the Following Images Only on the First Iteration
        ...
        Cexact = interp2d(X[0,:], Z[:,0], C, kind='cubic')(xrecon, zrecon);
        dslowness = 1/Cexact - 1/cbfm;
        dtimes = np.reshape(diff_times(path_integrate(dslowness.flatten(order='F'))), aberration_times.shape, order='F');
        xpos_idx = 80;
        # Visualize Sound Speed Map and Transducer Grid
        plt.figure(figsize=(6,6))
        imagesc(1000*X[0,:], 1000*Z[:,0], C, [np.min(C), np.max(C)]); 
        plt.xlabel('Lateral [mm]'); plt.ylabel('Axial [mm]'); plt.colorbar();
        plt.plot(1000*xpos, np.zeros(xpos.shape), 'w.', linewidth=2);
        plt.plot(1000*X_MSMTS, 1000*Z_MSMTS, 'r.', linewidth=2);
        plt.xlim(1000*np.array([min(x),max(x)])); plt.ylim(1000*np.array([min(z),max(z)])); 
        plt.gca().invert_yaxis(); plt.savefig('figures/SoSMapAndMeasurementGrid.png'); 
        # Visualize Differential Travel Times
        plt.figure(figsize=(9,4))
        plt.subplot(1,2,1); imagesc(1000*x_msmts, 1000*z_msmts, dtimes[:,:,xpos_idx-1], 
            [np.min(dtimes[:,:,xpos_idx-1]), np.max(dtimes[:,:,xpos_idx-1])]);
        plt.xlabel('Lateral [mm]'); plt.ylabel('Axial [mm]'); plt.title('Linearized Prediction'); plt.colorbar(); 
        plt.subplot(1,2,2); imagesc(1000*x_msmts, 1000*z_msmts, aberration_times[:,:,xpos_idx-1], 
            [np.min(dtimes[:,:,xpos_idx-1]), np.max(dtimes[:,:,xpos_idx-1])]);
        plt.xlabel('Lateral [mm]'); plt.ylabel('Axial [mm]'); plt.title('Actual Aberration Delay [s]'); plt.colorbar();
        ...
        plt.savefig('figures/AberrationDelayMeasurement.png'); 
    ...
    plt.figure(figsize=(9,4))
    plt.subplot(1,2,1); imagesc(1000*X[0,:], 1000*Z[:,0], C, [np.min(C),np.max(C)]); 
    plt.colorbar(); plt.xlabel('Lateral [mm]'); plt.ylabel('Axial [mm]'); 
    plt.title('Ground-Truth Speed of Sound'); 
    plt.xlim(1000*np.array([np.min(x),np.max(x)])); 
    plt.ylim(1000*np.array([np.min(z),np.max(z)]));
    plt.gca().invert_yaxis(); 
    plt.subplot(1,2,2); imagesc(1000*xrecon, 1000*zrecon, Crecon, [np.min(C),np.max(C)]); 
    plt.colorbar(); plt.xlabel('Lateral [mm]'); plt.ylabel('Axial [mm]'); 
